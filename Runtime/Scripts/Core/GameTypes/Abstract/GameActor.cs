...
namespace core.gameplay
{
    ...
    public abstract class GameActor : MonoBehaviour
    {
        ...
        {
            InputManager.onActionPressed(action, _logicAction, _map);
        ...
        {
            InputManager.onActionHold(action, _logicAction, _map);
        ...
        {
            InputManager.onActionReleased(action, _logicAction, _map);
                ...
                foreach (InputReference input_ref in _input_callbacks[map])
                {
                    switch (input_ref.callback_type)
                    {
                        case InputCallbackType.Pressed:
                            {
                                InputManager.UnsubscribeToActionPressed(input_ref.action, input_ref.callback, map);
                                break;
                            }
                        case InputCallbackType.Hold:
                            {
                                InputManager.UnsubscribeToActionHold(input_ref.action, input_ref.callback, map);
                                break;
                            }
                        case InputCallbackType.Released:
                            {
                                InputManager.UnsubscribeToActionReleased(input_ref.action, input_ref.callback, map);
                                break;
                            }
                    }
                }
